<x-admin-layout>
    <div class="bg-white rounded-lg shadow max-w-5xl">
        <div class="px-6 py-4 border-b border-gray-100">
            <h1 class="text-2xl font-semibold text-gray-800">Editar articulo</h1>
        </div>

        <form id="formEditar" method="POST" action="{{ route('posts.update', $post) }}">
            @csrf
            @method('PUT')
            <div class="px-6 py-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2 md:items-start">
                    ...
                    <div class="md:col-span-2">
                        <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $post->titulo) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2 md:items-start">
                    ...
                    <div class="md:col-span-2">
                        <textarea name="contenido" id="contenido" rows="4" required
                                  class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('contenido', $post->contenido) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2 md:items-start">
                    ...
                    <div class="md:col-span-2">
                        <input type="text" name="autor" id="autor" value="{{ old('autor', $post->autor) }}" required
                               class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2 md:items-start">
                    ...
                    <div class="md:col-span-2">
                        <input type="text" name="categoria" id="categoria" value="{{ old('categoria', $post->categoria) }}" required
                               class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-lg">
            <form method="POST" action="{{ route('posts.destroy', $post) }}"
                ...
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700">Eliminar</button>
            </form>
            <div class="flex gap-2">
                ...
                <button type="submit" form="formEditar" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Actualizar</button>
            </div>
        </div>
    </div>
</x-admin-layout>